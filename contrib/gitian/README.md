...
*Setup instructions for a Gitian build of Triton using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Triton CLI
...
```bash
cp triton/contrib/gitian/gitian-build.py .
```
...
gpg --output $VERSION-linux/$NAME/triton-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/triton-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/triton-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/triton-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/triton-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/triton-win-$VERSION-build.assert
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/triton-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/triton-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/triton-osx-*-build.assert
```
