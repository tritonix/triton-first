...
Core tests take longer than any other Triton tests, due to the high amount of computational work involved in validating core components.
...
To run only Triton's core tests (after building):
...
To run only Triton's crypto tests (after building):
...
```
tritond --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `triton-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Triton's hash tests (after building):
...
To run only Triton's performance tests (after building):
...
To run only Triton's unit tests (after building):
